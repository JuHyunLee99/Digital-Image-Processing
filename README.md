...
Fourier 스펙트럼 값들에 먼저 로그변환을 적용한 후, 새 범위를 선형적으로 스케일링해서 똑같은 8비트 표시기에 스펙트럼을 표시한 결과.
=> 풍부한 디테일을 확인할 수 있음.
(근데 이미 original_image는 Fourier 스펙트럼을 8비트로 선형적으로 스케일링한 이미지임. 원본 데이터의 정확한 동적 범위를 알 수는 없음.
사실, 이 이미지에 로그 변환을 적용하는 것은 원본 데이터의 동적 범위를 복원하는 것이 아니라, 단순히 이미 압축된 범위를 다시 변환하는 것에 불과한거 같음.  
따라서, 이 실습은 실질적으로 `0 ~ 1.5 * 10^6` 범위를 `0 ~ 6.2`로 변환한게 아님. 
로그변환 실습을 위한 참고로만...)

아.. 잠만..... 선형적으로 스케일링 한 이미지면 역스케일링으로 복원하면 정확하지는 않지만 근사치로 원본 스펙트럼 얻을 수 있는거 아님? 해보고 오기
